...
if exists("c_no_bracket_error")
  syn region	cParen		transparent start='(' end=')' contains=ALLBUT,@cParenGroup,cCppParen,cCppString fold
...
if exists("c_comment_strings")
  ...
  syntax region  cCommentL	start="//.*" end="\(^\s*\(//\)\@!\)\|\(^\s*$\)" contains=@cCommentGroup,cComment2String,cCharacter,cNumbersCom,cSpaceError fold
  syntax region cComment	matchgroup=cCommentStart start="/\*" end="\*/" contains=@cCommentGroup,cCommentStartError,cCommentString,cCharacter,cNumbersCom,cSpaceError fold
else
  syn region	cCommentL	start="//" end="\(^\s*\(//\)\@!\)\|\(^\s*$\)" contains=@cCommentGroup,cSpaceError fold
  syn region	cComment	matchgroup=cCommentStart start="/\*" end="\*/" contains=@cCommentGroup,cCommentStartError,cSpaceError fold
endif
...
if !exists("c_no_w32") " Win32
  syn keyword	cType		AULONG PULONG USHORT PUSHORT UCHAR PUCHAR PSZ
  syn keyword	cType		DWORD BOOL BYTE WORD FLOAT
  syn keyword	cType		PFLOAT PBOOL LPBOOL PBYTE LPBYTE PINT LPINT
  syn keyword	cType		PWORD LPWORD LPLONG PDWORD LPDWORD 
  syn keyword	cType		LPVOID LPCVOID INT UINT PUINT SIZE_T
  syn keyword	cType		LPSTR LPCSTR TCHAR LPTSTR LPCTSTR
  syn keyword	cType		WPARAM LPARAM LRESULT
  syn keyword	cType		HANDLE HWND HHOOK HEVENT ATOM
  syn keyword	cType		SPHANDLE LPHANDLE HGLOBAL HLOCAL 
  syn keyword	cType		GLOBALHANDLE LOCALHANDLE
  syn keyword	cType		FARPROC NEARPROC PROC
  syn keyword	cType		HGDIOBJ HKEY HACCEL HBITMAP HBRUSH 
  syn keyword	cType		HCOLORSPACE HDC HGLRC HDESK HENHMETAFILE 
  syn keyword	cType		HFONT HICON HMENU HMETAFILE HINSTANCE HMODULE
  syn keyword	cType		HPALETTE HPEN HRGN HRSRC HSTR HTASK HWINSTA 
  syn keyword	cType		HKL HMONITOR HWINEVENTHOOK HUMPD HFILE HCURSOR
  syn keyword	cType		COLORREF LPCOLORREF	
  syn keyword	ctype		RECT PRECT NPRECT LPRECT LPCRECT
  syn keyword	ctype		RECTL PRECTL LPRECTL LPCRECTL
  syn keyword	ctype		POINT PPOINT NPPOINT LPPOINT
  syn keyword	ctype		POINTL PPOINTL LPPOINTL
  syn keyword	ctype		POINTS PPOINTS LPPOINTS
  syn keyword	ctype		SIZE PSIZE LPSIZE
  syn keyword	ctype		SIZEL PSIZEL LPSIZEL
  syn keyword	cStorageClass	IN OUT OPTIONAL NEAR FAR CONST
  syn keyword	cStorageClass	CDECL CALLBACK WINAPI WINAPIV 
  syn keyword	cStorageClass	APIENTRY APIPRIVATE PASCAL
  syn keyword	cOperator	MAKEWORD MAKELONG LOWORD HIWORD LOBYTE HIBYTE	
  syn keyword	cConstant	TRUE FALSE HFILE_ERROR
endif
...
if !exists("c_no_if0")
  ...
  syn region	cCppOut2	contained start="0" end="^\s*\(%:\|#\)\s*\(endif\>\|else\>\|elif\>\)" contains=cSpaceError,cCppSkip fold
...
syn region cRegion 		start="/\*\s*region\>.*\*/" end="/\*\s*endregion\s*\*/" transparent fold keepend extend
