...
syn region  javaComment		 start="/\*"  end="\*/" fold contains=@javaCommentSpecial,javaTodo,@Spell
syn region  javaLineComment	 start="//.*"  end="\(^\s*\(//\)\@!\)\|\(^\s*$\)" fold contains=@javaCommentSpecial2,javaTodo,@Spell
...
syn region javaRegion 		start="//\s*region\>.*" end="//\s*endregion" transparent fold keepend extend
